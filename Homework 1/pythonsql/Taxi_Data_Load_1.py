...
def main(params):
    ...
    ny_taxi_pd_iter = pd.read_csv(filepath_or_buffer='green_tripdata_2019-01.csv.gz',iterator=True,chunksize=10000,compression='gzip')
    
    ny_taxi_pd = next(ny_taxi_pd_iter)
    
    ny_taxi_pd.lpep_pickup_datetime = pd.to_datetime(ny_taxi_pd.lpep_pickup_datetime)
    ny_taxi_pd.lpep_dropoff_datetime = pd.to_datetime(ny_taxi_pd.lpep_dropoff_datetime)
